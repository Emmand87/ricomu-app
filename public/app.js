...
function resetAll() {
  ...
  ['v_number','v_authority','v_article','v_place','v_dateInfrazione','v_dateNotifica','v_amount','v_targa','u_name','u_comune','u_dob','u_addr','u_cf'].forEach(id => { const i=el(id); if(i) i.value=''; });
  ['m_number','m_authority','m_article','m_place','m_dateInfrazione','m_dateNotifica','m_amount','m_targa','m_name','m_comune','m_dob','m_addr','m_cf'].forEach(id => { const i=el(id); if(i) i.value=''; });
...
/* ===== Campi chiave: punteggio ===== */
function fieldsScore(v = {}) {
  let score = 0;
  if (v.number) score++;
...
function showFieldsWarning(score){
  // Mostra avviso se 3/4 (prosegui ma “meno efficace”)
  // Nascondi se 4/4 (tutto ok) o <3 (mostreremo fallback)
  if (score === 3) show('fieldsWarn'); else hide('fieldsWarn');
}
...
function renderSummary() {
  ...
  const cites = (arr = []) =>
    arr.map(ci => `<small class="muted">[${ci.ref || ''}${ci.link ? ` – <a href="${ci.link}" target="_blank" rel="noopener">fonte</a>` : ''}]</small>`).join(' ');
  const centralHtml = c
  ...
  const btn = document.getElementById('openManualFromCentral'); if (btn) btn.onclick = openManualModal;
}
...
async function startCamera() {
  try {
    ...
    state.scan.images = []; el('scanThumbs').innerHTML = '';
...
function drawCurrentFrame(){
  ...
  ctx.putImageData(img,0,0); return c.toDataURL('image/jpeg',0.92);
}
...
async function onFinishScan(){
  ...
  const first=state.scan.images[0]; const blob=b64toBlob(first);
...
async function onHeroFileChange(){
  ...
  showLoader('Elaborazione in corso…','Stiamo analizzando il verbale con AI e OCR.<br/>Attendi ~1 minuto.');
  try{ const res=await fetch('/api/upload',{method:'POST',body:fd}); const data=await res.json(); await afterExtract(data); el('heroStatus').textContent='File elaborato.'; }
...
/* ===== Anteprima auto (usa fallbackMode se manca centralMotivo) ===== */
