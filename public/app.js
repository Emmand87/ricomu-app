...
function renderSummary() {
  ...
  // Se il motivo centrale NON c'è, mostra avviso + apri form manuale
  if (!c) {
    show('centralFallback');
    // apre direttamente il form
    openManualModal();
  } else {
    hide('centralFallback');
  }
}

/* ===== Validazione “sufficiente per anteprima” ===== */
function isDataSufficient() {
  const v = state.verbale || {};
  const c = state.motivi?.centralMotivo;
  // campi minimi + motivo centrale presente
  const hasCoreFields = Boolean((v.number || v.article || v.place) && (v.authority || v.place));
  return Boolean(c && hasCoreFields);
}
...
async function startCamera() {
  try {
    ...
    const video = el('camVideo'); video.srcObject = stream; await video.play();

    state.scan.images = []; el('scanThumbs').innerHTML = ''; el('scanStatus').textContent = 'Inquadra il verbale e premi “Scatta”.';
...
function drawCurrentFrame(){
  const video=el('camVideo'), canvas=el('camCanvas'); const w=video.videoWidth, h=video.videoHeight; if(!w||!h) return null;
  ...
  ctx.putImageData(img,0,0); return canvas.toDataURL('image/jpeg',0.92);
}
...
async function onFinishScan(){
  ...
  const first=state.scan.images[0]; const blob=b64toBlob(first); const fd=new FormData(); fd.append('file', new File([blob],'scan.jpg',{type:'image/jpeg'}));
  showLoader('Elaborazione in corso…','Stiamo analizzando il verbale con AI e OCR.<br/>Attendi ~1 minuto.');
  try{
    const res=await fetch('/api/upload',{method:'POST',body:fd}); const data=await res.json(); await afterExtract(data); el('scanStatus').textContent='Scansione inviata.';
  }catch(err){ console.error(err); el('scanStatus').textContent='Errore durante l’elaborazione.'; alert('Errore durante l’elaborazione. Riprova.'); }
...
async function onHeroFileChange(){
  const file=el('heroFile').files[0]; if(!file) return;
  el('heroStatus').textContent='Elaboro...'; show('workspace'); hideExtractionFallback();
  const fd=new FormData(); fd.append('file',file);
  showLoader('Elaborazione in corso…','Stiamo analizzando il verbale con AI e OCR.<br/>Attendi ~1 minuto.');
  try{
    const res=await fetch('/api/upload',{method:'POST',body:fd}); const data=await res.json(); await afterExtract(data); el('heroStatus').textContent='File elaborato.';
  }catch(err){ console.error(err); el('heroStatus').textContent='Errore durante l’elaborazione.'; alert('Errore durante l’elaborazione. Riprova.'); }
...
async function afterExtract(data){
  ...
  // 4) se i dati sono sufficienti → genera anteprima AUTO; altrimenti mostra avviso + form
  if(isDataSufficient()){
    await generatePreview(); // niente bottone, parte da sé
  } else {
    show('centralFallback');
    openManualModal();
  }
...
async function saveCorrections(){
  ...
  if(isDataSufficient()){
    ...
    hide('step5'); hide('step6'); hide('step7'); // niente anteprima finché non ok
    show('centralFallback');
  }
}

/* ===== Anteprima auto (no testi extra, no bottone) ===== */
async function generatePreview(){
  // genera testo ricorso
  const resAI = await fetch('/api/ai/genera-ricorso',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({verbale:state.verbale})});
  ...
  // watermark (resta, ma senza testo descrittivo nella UI)
  ...
  // timer 30s (solo valore numerico)
