// app.js
let state = {
  verbale:null, motivi:null, token:null,
...
function renderSummary(){
  ...
  const centralHtml = c ? `<p><span class="btn btn-outline" style="cursor:default">Motivo centrale (AI): ${c.type}${c.detail ? ' – '+c.detail : ''}</span> ${cites(c.citations)}</p>` : '<p class="muted">Motivo centrale non determinato.</p>';
...
function drawCurrentFrame(){
  ...
  // filtri leggeri “scanner”: aumento contrasto/mono
...
function onAddPage(){ show('btnShot'); }
async function onFinishScan(){
  // invia ogni pagina come upload separato: tieni la più leggibile (prima) per l’estrazione
  ...
  // Invia solo la prima per l’estrazione (più semplice). Potresti iterare e concatenare i testi.
...
async function computeMotiviAI(){
  // chiama il motivo centrale (AI) tramite le tue rotte già deployate
...
async function afterExtract(data){
  // qui puoi mappare il parser avanzato se presente. Per ora manteniamo compatibilità:
...
async function generatePreview(){
  // qui puoi riusare la tua rotta già esistente di generazione ricorso
...
document.getElementById('btnShot').onclick=onShot;
document.getElementById('btnRetake').onclick=onRetake;
document.getElementById('btnAddPage').onclick=onAddPage;
document.getElementById('btnFinishScan').onclick=onFinishScan;

// chiudi camera quando navighi via (best effort)
